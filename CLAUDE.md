# Entity_Starter Types

Shared TypeScript type definitions for the Entity_Starter API ecosystem.

**npm**: `@sudobility/entityentitystarter_types` (public)
...
```
@sudobility/entityentitystarter_types (this package)
    ^
entityentitystarter_api
entityentitystarter_client
entityentitystarter_lib
entityentitystarter_app
entityentityentitystarter_app_rn
```

Foundation layer — all other Entity_Starter projects depend on this package.

## Related Projects

- **entityentitystarter_api** — Backend API server (Hono + PostgreSQL); imports types for request/response validation
- **entityentitystarter_client** — API client SDK with TanStack Query hooks; imports types for API contracts
- **entityentitystarter_lib** — Business logic library with Zustand stores; imports types transitively via entityentitystarter_client
- **entityentitystarter_app** — Web application (React + Vite); imports types transitively via entityentitystarter_client and entityentitystarter_lib
- **entityentityentitystarter_app_rn** — React Native mobile app (Expo); imports types via file: links

All other entitystarter_* projects depend on this package. This package depends on `@sudobility/types` for base infrastructure types (`BaseResponse`, `NetworkClient`, etc.).
...
- Changes here affect ALL consumer projects (entityentitystarter_api, entityentitystarter_client, entityentitystarter_lib, entityentitystarter_app, entityentityentitystarter_app_rn) -- always consider downstream impact
